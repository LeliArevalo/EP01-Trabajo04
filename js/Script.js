window.onload = function()
{
    ...
    */
    ...
    //nom_div("ValidarSudoku").addEventListener('click', function(event)
      //{
        var validaSudoku = function (dato, id){
          var parte = id.split("_");
//console.table(solve[0][0]);
console.log(nom_div(id).value);
console.log(dato);
console.table(solve[parte[0]][parte[1]]);

          /*for (var i=0; i<solve[parte[0]][parte[1].length; i++)
          {
            for (var j=0; j<solve[parte[0]][parte[1].length;j++)
            {*/
              if(dato === solve[parte[0]][parte[1]][parte[2]][parte[3]]){
              /*  for (var k=0; k<sudoku.length;k++){
                  if(k === sudoku){
                */    console.log("Sudoku completado");
                nom_div(id).style.background=" #32CD32";
                  //}
                //}
              }else
              {
                console.log("error");
                nom_div(id).style.background="#FF7F50";
              }
                //}
              //}
          //  })();
          if (dato === 0) {
          nom_div(id).style.background=" rgba(221, 221, 221, 0.16)";
          }
         }
